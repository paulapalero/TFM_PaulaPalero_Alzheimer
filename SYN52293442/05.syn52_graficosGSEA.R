...
# LOS DATOS SCE DE SYN52293442
# GRÁFICOS RESUMEN GSEA
########################################################
...
dirs <- list(
  GO = "/clinicfs/userhomes/ppalero/SYN52293442/GSEA/GO",
  Reactome = "/clinicfs/userhomes/ppalero/SYN52293442/GSEA/Reactome",
  Aging = "/clinicfs/userhomes/ppalero/SYN52293442/GSEA/Aging"
)
...
generate_plots <- function(directory, analysis_name) {
  ...
  for (cell in cell_types) {
    ...
    p <- ggplot(plot_data, aes(x = Group, y = Description)) +
      # Capa única de puntos usando la columna is_sig para la forma
    ...
    output_file <- file.path(output_dir, paste0("Dotplot_", analysis_name, "_", cell, ".png"))
    ggsave(output_file, p, width = 10, height = 10, dpi = 300, bg = "white")
  }
}

# Executar per a ambos directorios/Database
