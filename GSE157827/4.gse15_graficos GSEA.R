...
dirs <- list(
  GO = "/clinicfs/userhomes/ppalero/GSE157827/GSEA/GO",
  Reactome = "/clinicfs/userhomes/ppalero/GSE157827/GSEA/Reactome",
  Aging = "/clinicfs/userhomes/ppalero/GSE157827/GSEA/Aging"
)
...
generate_plots <- function(directory, analysis_name) {
  ...
  for (cell in cell_types) {
    ...
    df_ad <- read.csv(file_ad) %>% mutate(Group = paste0(cell, " (AD)"))
    df_ctrl <- read.csv(file_ctrl) %>% mutate(Group = paste0(cell, " (Control)"))
    
    # Ordenamos por p.adjust (menor a mayor) 
    # Y desempatamos (si tienen el mismo p.adjust) por el valor absoluto de 
    # NES (mayor a menor)
