...
input_dir <- "/clinicfs/userhomes/ppalero/GSE157827/DEA/Wilcoxon"
base_out  <- "/clinicfs/userhomes/ppalero/GSE157827"
...
# 4. WikiPathways Aging
wp_anotacion <- read.gmt(file.path(genesets_path, "wikipathways-20260410-gmt-Homo_sapiens.gmt"))
aging_wp <- wp_anotacion[grepl("aging|senescence", wp_anotacion$term, ignore.case = TRUE), ]
colnames(aging_wp) <- c("Term", "Gene")
...
for (file_path in files) {
  ...
  res_reac <- GSEA(geneList = geneList, 
                   TERM2GENE = T2G_REAC, 
                   TERM2NAME = T2N_REAC,
                   nPermSimple = 10000, # número de permutaciones aleatorias Permite detectar p-valores más significativos
                   minGSSize = 10, 
                   maxGSSize = 500, 
                   pvalueCutoff = 1, 
                   verbose = FALSE)
  ...
  res_aging <- GSEA(geneList = geneList, 
                    ...
                    TERM2NAME = T2N_AGING,
                    nPermSimple = 10000, 
                    minGSSize = 5,   # Algunos sets de aging son pequeños
                    maxGSSize = 500, 
                    pvalueCutoff = 1, 
                    verbose = FALSE)
